...
class LabelEmbedder(nn.Module):
    ...
    def token_drop(self, labels, force_drop_ids=None):
        ...
        labels = torch.where(drop_ids, self.num_classes, labels) # 将num_classes中满足 drop_ids 全部赋值为 labels
        return labels
...
class DiffusionTranformer(nn.Module):
    def __init__(
        self,
        input_size=32,
        patch_size=2,
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale):
        ...
        #TODO: 需要进一步了解
        half = x[: len(x) // 2] # 前一半用于 有条件预测 后一半用于无条件预测
...
if __name__ == "__main__":
    ...
    model = DiffusionTranformer(**params_dit)
    model.to(device)
    out = model(image, t, y)
    print(out.shape)